...
files <- list.files(path = here("Lab_4", "dat"), 
                      pattern = "*EPA", full.names = TRUE)

ej_reports <- lapply(files, pdf_text)

ej_pdf <- readtext(file = here("Lab_4", "dat", "*.pdf"), 
...
par_tokens <- unnest_tokens(raw_text, output = paragraphs, input = text, token = "paragraphs") #getting out the paragraphs 

par_tokens <- par_tokens %>%
 mutate(par_id = 1:n()) #giving it an id variable 

par_words <- unnest_tokens(par_tokens, output = word, input = paragraphs, token = "words") #table with paragrah id and the individual words in it
...
```{r hierarch_clust}
dist <- as.dist(textstat_dist(dfm2))
clust <- hclust(dist)
